...
object MergeSort {
  ...
  def main(args: Array[String]): Unit = {
    (10000000 to 10000000).foreach { i ⇒
      val vec = (0 to i).toList.map((_: Int) ⇒ Random.nextInt(10000000))
...
//      println(res == res4)
  ...
  def parallelSort(values: List [Int],
    ...
    if (start > end) Future { List() }
    else if (depth > maxDepth / 2 || start + end < depth || start == end) {
      Future { values.sorted }
    } else {
      val step = values.size / maxDepth
      Future.sequence((0 to values.size + 1 by step).map(start ⇒
        if (start + step >= values.size)
        Future {
          values.slice(start, values.size).sorted
        }
        else Future { values.slice(start, start + step)}
      )).map(_.flatten.toList.sorted)
    }
  }
  ...
  def mergeSortParallel(values: List[Int],
    ...
    if (start > end) Future { List() }
      ...
      val f1: Future[List[Int]] = mergeSortParallel(values, start, middle, depth + 1)
      val f2: Future[List[Int]] = mergeSortParallel(values, middle + 1, end, depth + 1)
      f1
        .flatMap(l1 ⇒
          f2
            .map(l2 ⇒ merge(l1, l2))
